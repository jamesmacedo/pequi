...
class ColorService {
  ...
  Future<void> generateColorsStatic(
    ...
    final outputResolved = await Isolate.resolvePackageUri(
      Uri.parse('package:pequi/src/generated/theme/$type.dart'),
    );

    if (outputResolved == null) {
      print('Not found: package:pequi/src/generated/theme/$type.dart');
      exit(1);
    }
    ...
    for (final color in colors.entries) {
      buffer.writeln(
          '  static const ${color.key} = Color(0xFF${_hex(color.value)});');
      if (i == 3) {
        break;
      }
    }
  ...
  Future<void> generateColorsFromYAML(String label) async {
    ...
    final outputResolved = await Isolate.resolvePackageUri(
      Uri.parse('package:pequi/src/generated/theme/brand.dart'),
    );

    if (outputResolved == null) {
      print('Not found: package:pequi/src/generated/theme/brand.dart');
      exit(1);
    }
    ...
    buffer.writeln(
        '  static const primary = Color(0xFF${_hex(colors['primary'])});');
    buffer.writeln(
        '  static const secondary = Color(0xFF${_hex(colors['secondary'])});');
    buffer.writeln(
        '  static const accent = Color(0xFF${_hex(colors['accent'])});');
    buffer.writeln('}');
  ...
  void generateColors(String label) {
    ...
    generateColorsStatic(
        colors: defaultColors, type: 'default', className: 'DefaultColors');
    generateColorsStatic(
        colors: typoColors, type: 'typo', className: 'TypoColors');
    generateColorsStatic(
        colors: feedbackColors, type: 'feedback', className: 'FeedbackColors');
  }
}
